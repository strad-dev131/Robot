...
from TEAMXMUSIC.utils.TEAMX_ban import admin_filter
from TEAMXMUSIC.mongo.notesdb import *
from TEAMXMUSIC.utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from TEAMXMUSIC.utils.yumidb import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. TEAMXMUSIC will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
