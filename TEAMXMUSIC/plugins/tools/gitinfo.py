...
async def TEAMXhub(_, message):
    text = message.text[len("/daxxhub") :]
    papaTEAMX(f"{text}").save(f"daxxhub_{message.from_user.id}.png")
    await message.reply_photo(f"daxxhub_{message.from_user.id}.png")
    os.remove(f"TEAMXhub_{message.from_user.id}.png")
####

@app.on_message(filters.command(["github", "git"]))
async def github(_, message):
    if len(message.command) != 2:
        await message.reply_text("/git strad-dev131")
        return
