...
from TEAMXMUSIC import app, Userbot
...
async def strcall(client, message):
    assistant = await group_assistant(TEAMX, message.chat.id)
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./TEAMXMUSIC/assets/call.mp3"), stream_type=StreamType().pulse_stream)
